...
export class Rabbit {
  ...
  private async setupChannels(): Promise<void> {
    ...
    await this.orderChannel.assertQueue(config.QUEUE_ORDERS_REQUEST);
  }

  public async sendMessage(message: any, queue = config.QUEUE_ORDERS_REQUEST): Promise<void> {
    if (!this.orderChannel) {
      throw new CustomError('Rabbit connection failed',500);
    }
    try {
      const messageBuffer = Buffer.from(JSON.stringify(message));
      await this.orderChannel.sendToQueue(queue, messageBuffer);
